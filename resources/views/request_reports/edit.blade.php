                    ...
                    <form method="POST" action="{{ route('request-reports.update', $requestReport) }}">
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="work_start" class="block mt-1 w-full" type="datetime-local"
                                name="work_start" :value="old('work_start', $requestReport->work_start ? $requestReport->work_start->format('Y-m-d\TH:i') : '')" required />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="work_end" class="block mt-1 w-full" type="datetime-local"
                                name="work_end" :value="old('work_end', $requestReport->work_end ? $requestReport->work_end->format('Y-m-d\TH:i') : '')" required />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="work_total" class="block mt-1 w-full" type="number"
                                name="work_total" :value="old('work_total', $requestReport->work_total)" required min="1" />
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="descript" name="descript" rows="5"
                                class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" required>{{ old('descript', $requestReport->descript) }}</textarea>
                        ...
                        <div class="mt-4">
                            ...
                            <select id="state" name="state" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">
                                <option value="1" {{ $requestReport->state == 1 ? 'selected' : '' }}>Aktivní</option>
                                <option value="0" {{ $requestReport->state == 0 ? 'selected' : '' }}>Neaktivní</option>
                            </select>
                        ...
                        <div class="mt-4">
                            ...
                            <select id="kind" name="kind" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">
                                <option value="1" {{ $requestReport->kind == 1 ? 'selected' : '' }}>Standardní</option>
                                <option value="2" {{ $requestReport->kind == 2 ? 'selected' : '' }}>Konzultace</option>
                                <option value="3" {{ $requestReport->kind == 3 ? 'selected' : '' }}>Vývoj</option>
                                <option value="4" {{ $requestReport->kind == 4 ? 'selected' : '' }}>Testování</option>
                            </select>
                            <x-input-error :messages="$errors->get('kind')" class="mt-2" />
                        </div>

                        <div class="flex items-center justify-end mt-4">
                            <a href="{{ route('requests.show', $requestReport->request) }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-800 uppercase tracking-widest hover:bg-gray-300 focus:bg-gray-300 active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 mr-3">
                                {{ __('Zrušit') }}
                            </a>
