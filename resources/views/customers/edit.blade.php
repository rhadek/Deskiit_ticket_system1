<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Upravit firmu: ') }} {{ $customer->name }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('customers.update', $customer) }}">
                        ...
                        <!-- Název firmy -->
                        <div>
                            <x-input-label for="name" :value="__('Název firmy')" />
                        ...
                        <!-- IČ -->
                        <div class="mt-4">
                            <x-input-label for="ic" :value="__('IČ')" />
                            <x-text-input id="ic" class="block mt-1 w-full" type="text" name="ic" :value="old('ic', $customer->ic)" required />
                        ...
                        <div class="mt-4">
                            ...
                            <select id="state" name="state" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">
                                <option value="1" {{ $customer->state == 1 ? 'selected' : '' }}>Aktivní</option>
                                <option value="0" {{ $customer->state == 0 ? 'selected' : '' }}>Neaktivní</option>
                            </select>
                        ...
                        <div class="mt-4">
                            ...
                            <select id="kind" name="kind" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">
                                <option value="1" {{ $customer->kind == 1 ? 'selected' : '' }}>Standardní</option>
                                <option value="2" {{ $customer->kind == 2 ? 'selected' : '' }}>VIP</option>
                                <option value="3" {{ $customer->kind == 3 ? 'selected' : '' }}>Korporátní</option>
                            </select>
