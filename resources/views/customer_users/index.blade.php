<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            @if(auth()->user()->kind == 3)
            <a href="{{ route('customer_users.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700">
                {{ __('Nový uživatel') }}
            </a>
            @endif
        </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($customerUsers as $user)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <a href="{{ route('customers.show', $user->customer) }}" class="text-blue-600 hover:text-blue-900">
                                                {{ $user->customer->name }}
                                            </a>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            ...
                                            @else
                                                <a href="{{ route('customer_users.show', $user) }}" class="text-indigo-600 hover:text-indigo-900">Detail</a>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
